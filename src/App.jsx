...
import { 
  ...
  MessageSquare, Headphones, Clock, CheckCircle, ArrowUpRight, Phone, MapPin, Trash2, Plus, Image as ImageIcon
} from 'lucide-react';
...
// --- HÀM XỬ LÝ ĐÓNG DẤU THỜI GIAN & ĐỊA ĐIỂM LÊN ẢNH ---
const processImageWithWatermark = (base64Str, storeInfo, maxWidth = 800) => {
  return new Promise((resolve) => {
    ...
    img.onload = () => {
      ...
      // Lấy vị trí GPS (Nếu trình duyệt hỗ trợ)
      const getGPSAndDraw = () => {
        if ("geolocation" in navigator) {
          navigator.geolocation.getCurrentPosition(
            (pos) => {
              const gpsStr = `GPS: ${pos.coords.latitude.toFixed(4)}, ${pos.coords.longitude.toFixed(4)}`;
              drawText(timeStr, gpsStr);
            },
...
export default function App() {
  ...
  // Handler Upload ảnh có Watermark
  const handlePhotoUpload = async (target, field = null) => {
    ...
    input.onchange = async (e) => {
      if (e.target.files[0]) {
        ...
        reader.onload = async (ev) => {
          // Thực hiện đóng dấu watermark
          const processed = await processImageWithWatermark(ev.target.result, selectedStoreId);
          if (target === 'complaint') {
            setComplaintForm(prev => ({ ...prev, images: [...prev.images, processed] }));
          } else if (target === 'resolve') {
            setResolveForm(prev => ({ ...prev, img: processed }));
          } else if (target === 'checklist') {
            setChecklists(prev => prev.map(item => item.id === field ? { ...item, photo: processed } : item));
          }
  ...
  const submitChecklist = async () => {
    if (!staffName || !signature || !checklists.every(c => c.completed && c.photo)) return alert("Hoàn thành Checklist & Ký tên!");
    setIsSubmitting(true);
    try {
  ...
  const submitComplaint = async () => {
    if (!staffName || !complaintForm.custName || !complaintForm.custPhone || complaintForm.images.length === 0) return alert("Nhập đủ tên CS, thông tin khách & ít nhất 1 ảnh!");
    setIsSubmitting(true);
    try {
      await addDoc(collection(db, 'artifacts', appId, 'public', 'data', 'complaint_cases'), {
        cs: staffName, store: selectedStoreId, custName: complaintForm.custName, custPhone: complaintForm.custPhone,
    ...
    <div className="max-w-md mx-auto h-screen bg-slate-50 flex flex-col overflow-hidden relative font-sans text-slate-900 border-x border-slate-200 shadow-2xl">
      ...
      <header className="bg-white px-6 pt-10 pb-5 border-b border-slate-100 flex justify-between items-center shrink-0 z-20 shadow-sm">
        ...
        <div className="flex space-x-2">
           <button onClick={() => setActiveTab('complaints')} className={`w-10 h-10 rounded-2xl flex items-center justify-center border transition-all ${activeTab === 'complaints' ? 'bg-red-500 text-white shadow-red-200 border-red-400' : 'bg-white text-slate-300 border-slate-100'}`}>
              <MessageSquare size={18} />
           </button>
           <button onClick={() => setActiveTab('reports')} className={`w-10 h-10 rounded-2xl flex items-center justify-center border transition-all ${activeTab === 'reports' ? 'bg-blue-500 text-white shadow-blue-200 border-blue-400' : 'bg-white text-slate-300 border-slate-100'}`}>
              <FileText size={18} />
           </button>
        </div>
      </header>
      ...
      <main className="flex-1 overflow-y-auto p-6 pb-32">
          ...
          <div className="space-y-4 animate-in fade-in duration-500">
            <div className={`bg-white p-5 rounded-[28px] border border-slate-100 shadow-sm space-y-4 ${isSubmitted ? 'opacity-60' : ''}`}>
              <div className="flex items-center space-x-3 text-left">
                ...
                <div className="flex-1"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-1">Tên người thực hiện</label><input type="text" placeholder="Họ tên..." value={staffName} disabled={isSubmitted} onChange={(e) => setStaffName(e.target.value)} className="w-full bg-slate-50 border-none rounded-xl p-2.5 text-sm font-bold outline-none"/></div>
              </div>
                ...
                <div className="space-y-4">
                  <div className="space-y-3">
                    <div className="bg-slate-50 p-3 rounded-2xl border border-slate-100">
                      <label className="text-[9px] font-black text-slate-400 uppercase block mb-1">Tên khách hàng</label>
                      <input type="text" placeholder="..." className="w-full bg-transparent border-none text-sm font-bold outline-none" value={complaintForm.custName} onChange={(e) => setComplaintForm({...complaintForm, custName: e.target.value})}/>
                    </div>
                    <div className="bg-slate-50 p-3 rounded-2xl border border-slate-100">
                      <label className="text-[9px] font-black text-slate-400 uppercase block mb-1">Số điện thoại</label>
                      <input type="tel" placeholder="..." className="w-full bg-transparent border-none text-sm font-bold outline-none font-mono" value={complaintForm.custPhone} onChange={(e) => setComplaintForm({...complaintForm, custPhone: e.target.value})}/>
                    </div>
                    <div className="bg-slate-50 p-3 rounded-2xl border border-slate-100">
                      <label className="text-[9px] font-black text-slate-400 uppercase block mb-1">Địa chỉ</label>
                      <input type="text" placeholder="..." className="w-full bg-transparent border-none text-sm font-bold outline-none" value={complaintForm.custAddress} onChange={(e) => setComplaintForm({...complaintForm, custAddress: e.target.value})}/>
                    </div>
                  </div>

                  <div className="space-y-3">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest flex justify-between">Ảnh có đóng dấu <span className="text-red-500 italic">{complaintForm.images.length} ảnh</span></label>
                    <div className="grid grid-cols-4 gap-2">
                       {complaintForm.images.map((img, i) => (
                         <div key={i} className="relative aspect-square rounded-xl overflow-hidden border border-slate-200"><img src={img} className="w-full h-full object-cover" /><button onClick={() => removeComplaintPhoto(i)} className="absolute top-1 right-1 bg-red-500/80 text-white rounded-full p-1"><X size={10}/></button></div>
                       ))}
                       <button onClick={() => handlePhotoUpload('complaint')} className="aspect-square rounded-xl border-2 border-dashed border-slate-200 bg-slate-50 flex flex-col items-center justify-center text-slate-300">
                          <ImageIcon size={24}/>
                  ...
                  <div className="bg-slate-50 p-3 rounded-2xl">
                    <label className="text-[9px] font-black text-slate-400 uppercase block mb-1">Nội dung khiếu nại</label>
                    <textarea rows="3" className="w-full bg-transparent border-none text-sm font-bold outline-none" value={complaintForm.note} onChange={(e) => setComplaintForm({...complaintForm, note: e.target.value})}></textarea>
                  </div>

                  <button onClick={submitComplaint} disabled={isSubmitting} className="w-full bg-red-600 text-white font-black py-4 rounded-[22px] shadow-xl flex items-center justify-center space-x-2">
                    {isSubmitting ? <RefreshCw className="animate-spin" size={20}/> : <MessageSquare size={20}/>}
                    <span className="uppercase text-xs tracking-widest">{isSubmitting ? "Đang xử lý ảnh..." : "BÁO CA XỬ LÝ"}</span>
                  </button>
                </div>
              </div>
            ) : (
                ...
                <div className="space-y-2">
                  ...
                  {checklists.map(c => (
                    <div key={c.id} className={`p-4 rounded-[22px] border flex items-center space-x-4 ${c.completed ? 'bg-green-50 border-green-100' : 'bg-white border-slate-100 shadow-sm'}`}>
                      ...
                      <button disabled={isSubmitted} onClick={() => handlePhotoUpload('checklist', c.id)} className={`p-3 rounded-2xl ${c.photo ? 'text-green-600 bg-green-100' : 'text-slate-400 bg-slate-50'}`}><Camera size={22} /></button>
                    </div>
                  ))}
                </div>
        ...
        {activeTab === 'complaints' && (
          <div className="space-y-4 text-left animate-in slide-in-from-right duration-500">
            <h3 className="text-lg font-black uppercase tracking-tighter flex items-center"><Headphones size={20} className="mr-2 text-red-500"/> KHIẾU NẠI KHÁCH HÀNG</h3>
            {complaintHistory.length === 0 ? (
              ...
              complaintHistory.map(cp => (
                <div key={cp.id} onClick={() => setSelectedComplaint(cp)} className="bg-white p-5 rounded-[32px] border border-slate-100 shadow-sm space-y-3 active:scale-95 transition-all cursor-pointer relative overflow-hidden">
                  ...
                  <p className="text-xs text-slate-600 font-bold line-clamp-2 bg-slate-50 p-3 rounded-2xl italic leading-relaxed">"{cp.note}"</p>
                  <div className="flex justify-between items-center pt-1"><span className="text-[9px] font-black text-slate-300 uppercase">{cp.date}</span><button className="flex items-center text-[10px] font-black text-blue-500 uppercase italic">Chi tiết <ChevronRight size={12}/></button></div>
                </div>
              ))
            )}
          </div>
        ...
        {activeTab === 'reports' && (
          <div className="space-y-4 text-left animate-in slide-in-from-left duration-500">
            <h3 className="text-lg font-black uppercase tracking-tighter flex items-center"><FileText size={20} className="mr-2 text-blue-500"/> LỊCH SỬ VẬN HÀNH</h3>
        ...
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-lg z-50 flex items-end justify-center">
          <div className="bg-white w-full max-w-md h-[94vh] rounded-t-[50px] flex flex-col shadow-2xl animate-in slide-in-from-bottom duration-300 overflow-hidden text-left">
            <div className="p-8 border-b border-red-50 flex justify-between items-center shrink-0">
              <div><h3 className="text-xl font-black text-red-600 uppercase italic leading-none">CHI TIẾT CA {selectedComplaint.status.toUpperCase()}</h3><p className="text-[10px] font-bold text-slate-400 uppercase mt-2">Báo bởi CS: {selectedComplaint.cs}</p></div>
            ...
            <div className="flex-1 overflow-y-auto p-8 space-y-8">
                ...
                <div className="space-y-3">
                   ...
                   <div className="flex space-x-2 overflow-x-auto pb-2 scrollbar-hide">
                      {selectedComplaint.images?.map((img, i) => (
                        <img key={i} src={img} className="h-64 w-52 object-cover rounded-3xl border-4 border-slate-50 flex-none shadow-sm" />
                      ))}
                   </div>
                </div>
                <div className="bg-slate-50 p-5 rounded-3xl italic text-sm text-slate-600 border border-slate-100 font-serif leading-relaxed">"{selectedComplaint.note}"</div>
              </section>
                ...
                <section className="pt-6 border-t-2 border-dashed border-slate-100 space-y-5">
                  ...
                  {selectedComplaint.status === 'Mới' ? (
                    <div className="bg-orange-50 p-5 rounded-[35px] border border-orange-100 space-y-4">
                       ...
                       <div className="space-y-2">
                         <input type="text" placeholder="Họ tên người nhận ca..." className="w-full bg-white border border-orange-200 rounded-xl p-3 text-sm font-bold outline-none" value={managerReceive.name} onChange={(e) => setManagerReceive({...managerReceive, name: e.target.value})}/>
                       ...
                       <button onClick={() => handleReceiveCase(selectedComplaint.id)} className="w-full bg-orange-500 text-white font-black py-4 rounded-[22px] shadow-lg flex items-center justify-center space-x-3 active:scale-95 transition-all"><Clock size={20}/><span className="uppercase text-xs tracking-widest">NHẬN CA XỬ LÝ NGAY</span></button>
                    </div>
                    ...
                    <div className="space-y-4">
                       ...
                       <div className="bg-slate-50 p-5 rounded-3xl border border-slate-200">
                         ...
                         <button onClick={() => handlePhotoUpload('resolve')} className={`mt-3 w-full py-4 rounded-2xl border-2 border-dashed flex items-center justify-center space-x-2 transition-all ${resolveForm.img ? 'bg-green-100 border-green-300 text-green-700' : 'bg-white border-slate-200 text-slate-400'}`}>
                           ...
                           <span className="text-[10px] font-black uppercase">{resolveForm.img ? "ĐÃ CÓ ẢNH ĐÓNG DẤU" : "CHỌN ẢNH KẾT QUẢ XỬ LÝ"}</span>
                         </button>
                       </div>
                       <div className="grid grid-cols-2 gap-3">
                         <button onClick={() => handleResolveCase(selectedComplaint.id, "Hoàn thành")} className="bg-green-600 text-white font-black py-4 rounded-2xl shadow-lg text-[10px] uppercase tracking-widest flex items-center justify-center space-x-2 active:scale-95 transition-all"><CheckCircle size={16}/><span>ĐÓNG CA</span></button>
                         <button onClick={() => handleResolveCase(selectedComplaint.id, "Chuyển CS")} className="bg-slate-900 text-white font-black py-4 rounded-2xl shadow-lg text-[10px] uppercase tracking-widest flex items-center justify-center space-x-2 active:scale-95 transition-all"><ArrowUpRight size={16}/><span>CHUYỂN CS</span></button>
                       </div>
                    </div>
                  ) : null}
                </section>
              ...
              {selectedComplaint.status !== 'Mới' && selectedRole === "Chăm sóc khách hàng (CS)" && (
                <div className="bg-blue-50 p-5 rounded-3xl border border-blue-100 space-y-1">
                   <p className="text-[9px] font-black text-blue-300 uppercase tracking-widest text-left">Quản lý nhận xử lý</p>
                   <p className="text-sm font-black text-blue-600 uppercase text-left">{selectedComplaint.manager} • {selectedComplaint.managerPhone}</p>
                </div>
              )}

              {(selectedComplaint.status === 'Hoàn thành' || selectedComplaint.status === 'Chuyển CS') && (
                <section className="pt-6 border-t-2 border-dashed border-slate-100 space-y-4 animate-in fade-in duration-700 text-left">
                   ...
                   <div className="bg-green-50 p-6 rounded-[32px] border border-green-100 space-y-4 shadow-sm text-left">
                      <p className="text-sm font-bold text-slate-700 italic leading-relaxed text-left">"{selectedComplaint.resolution}"</p>
