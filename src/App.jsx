...
const App = () => {
  ...
  const handleComplete = async () => {
    ...
    try {
      ...
      alert("Lỗi Firestore: " + error.code + " - " + error.message);
    } finally {
  ...
  const handleReportIssue = async () => {
    ...
    } catch (error) { alert("Lỗi gửi báo cáo: " + error.message); } finally { setIsSubmitting(false); }
  };
      ...
      <main className="flex-1 overflow-y-auto p-6 pb-32">
        {activeTab === 'dashboard' && (
          <div className="space-y-6">
            <div className="bg-gradient-to-br from-orange-500 to-orange-600 p-6 rounded-[32px] text-white shadow-xl shadow-orange-100">
            ...
            <div className="space-y-3">
               <h4 className="text-[11px] font-black text-slate-400 uppercase ml-1 tracking-wider text-left text-left">Hoạt động Live</h4>
          ...
          <div className="space-y-4">
            <div className={`bg-white p-5 rounded-[28px] border border-slate-100 shadow-sm space-y-4 ${isSubmitted ? 'opacity-70' : ''}`}>
              ...
              <div className="flex items-center space-x-3 border-t border-slate-50 pt-3 text-left">
                ...
                <div className="flex-1 text-left"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-1 text-left">Bộ phận trực</label><select className="w-full bg-slate-50 border-none rounded-xl p-2 text-sm font-bold outline-none text-left" value={selectedRole} disabled={isSubmitted} onChange={(e) => setSelectedRole(e.target.value)}>{ROLES.map(r => <option key={r} value={r}>{r}</option>)}</select></div>
              </div>
            </div>
        ...
        {activeTab === 'issues' && (
          <div className="flex flex-col items-center justify-center h-full text-center space-y-6">
            ...
            <div className="text-center"><h3 className="font-black text-xl text-slate-900 uppercase tracking-tighter">Báo cáo lỗi QC</h3><p className="text-slate-400 text-sm px-10 mt-2 italic text-center">Mọi sự cố sẽ được gửi trực tiếp cho Founder.</p></div>
            <button onClick={() => setShowIssueModal(true)} className="bg-slate-900 text-white font-black px-10 py-4 rounded-2xl shadow-xl uppercase text-xs active:scale-95 transition-transform tracking-widest text-center">Báo cáo lỗi mới</button>
          </div>
        )}
      </main>
