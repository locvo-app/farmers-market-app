...
export default function App() {
  ...
  if (profile?.status === 'pending' && profile?.phone !== ADMIN_PHONE) return (
    <div className="max-w-md mx-auto h-screen bg-slate-50 flex flex-col items-center justify-center p-8 text-center space-y-6">
      ...
      <div className="space-y-2 text-left"><h2 className="text-xl font-black text-slate-900 uppercase">Chờ phê duyệt</h2><p className="text-slate-500 text-sm font-bold px-6 leading-relaxed">Chào {profile.fullName}, tài khoản Farmers của bạn đang chờ Admin duyệt. Vui lòng quay lại sau!</p></div>
            ...
            {selectedRole === "Chăm sóc khách hàng (CS)" ? (
               ...
               <div className="space-y-4 text-left">
                  <div className="space-y-2 text-left">
                    <h4 className="text-[11px] font-black text-slate-400 uppercase ml-1 tracking-widest text-left text-left text-left">Checklist ca trực</h4>
                    {checklists.map(c => (
                      <div key={c.id} className={`p-4 rounded-[22px] border flex items-center space-x-4 text-left ${c.completed ? 'bg-green-50 border-green-100' : 'bg-white border-slate-100 shadow-sm'}`}>
                        ...
                        <div className="flex-1 text-left text-sm font-bold text-slate-700 text-left">{c.task}</div>
                        <button disabled={isSubmitted} onClick={() => handlePhotoUpload('checklist', c.id, selectedRole !== "Chăm sóc khách hàng (CS)")} className={`p-3 rounded-2xl ${c.photo ? 'text-green-600 bg-green-100 shadow-inner' : 'text-slate-400 bg-slate-50'} text-left`}><Camera size={22} /></button>
                      </div>
                    ))}
                  </div>
                  <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-inner text-left">
                    ...
                    <div className="h-32 bg-slate-50 border-2 border-dashed border-slate-200 rounded-xl overflow-hidden touch-none text-left">
                ...
                <div className="space-y-3 text-left">
                    {usersList.filter(u => u.status === 'pending').map(u => (
                        <div key={u.id} className="bg-white p-4 rounded-3xl border border-slate-100 shadow-sm flex items-center justify-between animate-in fade-in text-left">
                            <div className="text-left text-left"><p className="text-sm font-black text-slate-800 text-left">{u.fullName}</p><p className="text-[10px] font-bold text-slate-400 uppercase font-mono text-left">{u.phone}</p></div>
                    ...
                    {usersList.filter(u => u.status === 'pending').length === 0 && <p className="text-slate-400 text-xs italic py-20 text-center text-left">Không có ai chờ duyệt.</p>}
                </div>
            </div>
        )}
