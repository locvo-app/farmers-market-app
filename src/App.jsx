    ...
    img.onload = () => {
      ...
      if ("geolocation" in navigator) {
        navigator.geolocation.getCurrentPosition(
          ...
          () => drawText(timeStr, "GPS: Vị trí chưa cấp quyền")
        );
...
export default function App() {
  ...
  // HÀM UPLOAD ẢNH CÓ KIỂM SOÁT CAMERA/GALLERY
  const handlePhotoUpload = async (target, field = null, forceCamera = false) => {
    const input = document.createElement('input');
    input.type = 'file'; 
    input.accept = 'image/*';
    // Chỉ ép dùng Camera nếu forceCamera = true
    if (forceCamera) {
  ...
  const submitChecklist = async () => {
    if (!staffName || !signature || !checklists.every(c => c.completed && c.photo)) return alert("Vui lòng nhập tên, hoàn thành việc & ký tên!");
    setIsSubmitting(true);
    try {
  ...
  const submitComplaint = async () => {
    ...
    try {
      ...
      alert("Đã báo ca khiếu nại!");
    } catch (e) { alert("Lỗi: " + e.message); } finally { setIsSubmitting(false); }
  };

  const handleReceiveCase = async (id) => {
    if (!managerReceive.name || !managerReceive.phone) return alert("Vui lòng điền Tên và SĐT người xử lý!");
    try {
      const ref = doc(db, 'artifacts', appId, 'public', 'data', 'complaint_cases', id);
      await updateDoc(ref, { 
        status: "Đang xử lý", 
        manager: managerReceive.name, 
        managerPhone: managerReceive.phone,
        updatedAt: serverTimestamp() 
      });
      setSelectedComplaint(null);
      setManagerReceive({ name: "", phone: "" });
    } catch (e) { alert("Lỗi: " + e.message); }
  };

  const handleResolveAction = async (id, status) => {
    if (!resolveForm.text) return alert("Vui lòng nhập kết quả!");
    setIsSubmitting(true);
    try {
      const ref = doc(db, 'artifacts', appId, 'public', 'data', 'complaint_cases', id);
      await updateDoc(ref, { 
        status, 
        resolution: resolveForm.text, 
        resolutionImg: resolveForm.img,
        updatedAt: serverTimestamp() 
      });
      setResolveForm({ text: "", img: "" }); 
      setSelectedComplaint(null);
      alert(status === 'Hoàn thành' ? "Đã đóng ca thành công!" : "Đã chuyển CS hỗ trợ!");
    } catch (e) { alert("Lỗi: " + e.message); } finally { setIsSubmitting(false); }
  };
  ...
  return (
    <div className="max-w-md mx-auto h-screen bg-slate-50 flex flex-col overflow-hidden relative font-sans text-slate-900 border-x border-slate-200">
      ...
      <main className="flex-1 overflow-y-auto p-6 pb-32">
          ...
          <div className="space-y-4 animate-in fade-in">
            <div className={`bg-white p-5 rounded-[28px] border border-slate-100 shadow-sm space-y-4 ${isSubmitted ? 'opacity-60' : ''}`}>
              <div className="flex items-center space-x-3 text-left">
                ...
                <div className="flex-1"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-1 text-left">Người thực hiện</label><input type="text" placeholder="Nhập tên..." value={staffName} disabled={isSubmitted} onChange={(e) => setStaffName(e.target.value)} className="w-full bg-slate-50 border-none rounded-xl p-2.5 text-sm font-bold outline-none"/></div>
              </div>
              <div className="flex items-center space-x-3 border-t border-slate-50 pt-3 text-left text-left">
                ...
                <div className="flex-1"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-1 text-left">Cửa hàng</label><select className="w-full bg-slate-50 border-none rounded-xl p-2 text-sm font-bold outline-none font-black" value={selectedStoreId} disabled={isSubmitted} onChange={(e) => setSelectedStoreId(e.target.value)}>{STORES_LIST.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}</select></div>
              </div>
              <div className="flex items-center space-x-3 border-t border-slate-50 pt-3 text-left text-left">
                ...
                <div className="flex-1"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-1 text-left text-left">Bộ phận / Vai trò</label><select className="w-full bg-slate-50 border-none rounded-xl p-2 text-sm font-bold outline-none" value={selectedRole} disabled={isSubmitted} onChange={(e) => setSelectedRole(e.target.value)}>{ROLES.map(r => <option key={r} value={r}>{r}</option>)}</select></div>
              </div>
            </div>
            ...
            {selectedRole === "Chăm sóc khách hàng (CS)" ? (
              <div className="bg-white p-6 rounded-[35px] border-2 border-red-50 shadow-2xl space-y-6 text-left animate-in zoom-in">
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-3">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest flex justify-between items-center text-left">Ảnh minh chứng (Thư viện/Chụp) <span className="text-red-500 italic">{complaintForm.images.length} ảnh</span></label>
              ...
              <div className="space-y-4">
                <div className="space-y-2">
                  ...
                  {checklists.map(c => (
                    <div key={c.id} className={`p-4 rounded-[22px] border flex items-center space-x-4 ${c.completed ? 'bg-green-50 border-green-100' : 'bg-white border-slate-100 shadow-sm'}`}>
                      ...
                      {/* PHÂN QUYỀN CAMERA Ở ĐÂY: CS được dùng gallery, người khác ép camera */}
                ...
                {!isSubmitted ? <button onClick={submitChecklist} disabled={isSubmitting} className="w-full bg-slate-900 text-white font-black py-5 rounded-[28px] shadow-2xl flex items-center justify-center space-x-2 uppercase tracking-widest italic active:scale-95 transition-all">{isSubmitting ? <RefreshCw className="animate-spin" /> : <Edit3 />} <span>Nộp/ Submit</span></button> : <div className="bg-green-500 p-5 rounded-[28px] text-white flex items-center justify-between shadow-lg font-black uppercase text-xs tracking-widest text-left text-left">Hoàn thành! <CheckCircle2 size={24} /></div>}
              </div>
            )}
          </div>
        )}
        ...
        {activeTab === 'reports' && (
          <div className="space-y-4 text-left animate-in slide-in-from-left">
            <h3 className="text-lg font-black uppercase flex items-center tracking-tighter"><FileText size={20} className="mr-2 text-blue-500"/> LỊCH SỬ CHECKLIST</h3>
            {submissionsHistory.length === 0 ? <p className="text-slate-400 text-sm italic py-20 text-center text-left">Chưa có báo cáo.</p> :
              submissionsHistory.map(sub => (
                <button key={sub.id} onClick={() => setSelectedReport(sub)} className="w-full bg-white p-5 rounded-[24px] border border-slate-100 shadow-sm flex items-center space-x-4 active:scale-95 transition-all text-left text-left text-left">
      ...
      {/* MODAL CHI TIẾT KHIẾU NẠI & XỬ LÝ */}
      {selectedComplaint && (
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-lg z-50 flex items-end justify-center">
          <div className="bg-white w-full max-w-md h-[94vh] rounded-t-[50px] flex flex-col shadow-2xl animate-in slide-in-from-bottom text-left">
            <div className="p-8 border-b border-red-50 flex justify-between items-center shrink-0">
              <div><h3 className="text-xl font-black text-red-600 uppercase italic leading-none">CHI TIẾT CA {selectedComplaint.status.toUpperCase()}</h3><p className="text-[10px] font-bold text-slate-400 uppercase mt-2">Báo bởi CS: {selectedComplaint.cs} • {selectedComplaint.date}</p></div>
            ...
            <div className="flex-1 overflow-y-auto p-8 space-y-8">
              ...
              {selectedRole === "Quản Lý Ca" && (
                <section className="pt-6 border-t-2 border-dashed border-slate-100 space-y-5">
                  <h4 className="text-[11px] font-black text-slate-800 uppercase tracking-widest flex items-center"><Edit3 size={14} className="mr-2 text-orange-500"/> NHÂN VIÊN XỬ LÝ</h4>
                  ...
                  {selectedComplaint.status === 'Mới' ? (
                    <div className="bg-orange-50 p-5 rounded-[35px] border border-orange-100 space-y-4">
                       ...
                       <button onClick={() => handleReceiveCase(selectedComplaint.id)} className="w-full bg-orange-500 text-white font-black py-4 rounded-[22px] shadow-lg flex items-center justify-center space-x-3 active:scale-95 transition-all italic"><Clock size={20}/><span className="uppercase text-xs tracking-widest italic">XÁC NHẬN NHẬN CA</span></button>
                    </div>
                  ) : selectedComplaint.status === 'Đang xử lý' ? (
                    <div className="space-y-4">
                       <div className="bg-blue-50 p-5 rounded-3xl border border-blue-100 space-y-2">
                          <p className="text-[9px] font-black text-blue-300 uppercase tracking-widest text-left">Đang được xử lý bởi</p>
                       ...
                       <div className="bg-slate-50 p-5 rounded-3xl border border-slate-200 text-left">
                         <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-2 text-left">Ghi chú kết quả xử lý thực tế</label>
                         ...
                         <button onClick={() => handlePhotoUpload('resolve')} className={`mt-3 w-full py-4 rounded-2xl border-2 border-dashed flex items-center justify-center space-x-2 transition-all ${resolveForm.img ? 'bg-green-100 border-green-300 text-green-700' : 'bg-white border-slate-200 text-slate-400'}`}>
                           ...
                           <span className="text-[10px] font-black uppercase">{resolveForm.img ? "ĐÃ CÓ ẢNH KẾT QUẢ" : "ẢNH MINH CHỨNG KẾT QUẢ"}</span>
                         </button>
                       </div>
                       <div className="grid grid-cols-2 gap-3 text-left">
                         <button onClick={() => handleResolveAction(selectedComplaint.id, "Hoàn thành")} className="bg-green-600 text-white font-black py-4 rounded-2xl shadow-lg text-[10px] uppercase tracking-widest flex items-center justify-center space-x-2 active:scale-95 transition-all"><CheckCircle size={16}/><span>Hoàn Thành</span></button>
                         <button onClick={() => handleResolveAction(selectedComplaint.id, "Chuyển CS")} className="bg-slate-900 text-white font-black py-4 rounded-2xl shadow-lg text-[10px] uppercase tracking-widest flex items-center justify-center space-x-2 active:scale-95 transition-all"><ArrowUpRight size={16}/><span>Chuyển CS</span></button>
                       </div>
                    </div>
                  ) : null}
                </section>
              ...
              {(selectedComplaint.status === 'Hoàn thành' || selectedComplaint.status === 'Chuyển CS') && (
                <section className="pt-6 border-t-2 border-dashed border-slate-100 space-y-4 animate-in fade-in text-left text-left text-left">
                   ...
                   <div className="bg-green-50 p-6 rounded-[32px] border border-green-100 space-y-4 shadow-sm text-left">
                      <p className="text-sm font-bold text-slate-700 italic leading-relaxed text-left text-left">"{selectedComplaint.resolution}"</p>
            ...
            <div className="flex-1 overflow-y-auto p-8 space-y-6">
              {selectedReport.taskList?.map((t, idx) => (
                <div key={idx} className="bg-slate-50 p-4 rounded-3xl border border-slate-100 text-left">
                  <div className="flex items-center space-x-3 mb-3 text-sm font-bold text-slate-700 text-left"><CheckCircle2 size={16} className="text-green-500" />{t.task}</div>
                  {t.img && <img src={t.img} className="w-full h-48 object-cover rounded-2xl border-white border shadow-sm" />}
                </div>
              ))}
              <div className="space-y-4 pt-4 border-t border-slate-50 text-left text-left text-left">
